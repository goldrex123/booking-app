...
const BookingsPage = () => {
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      const vehicleBookings = bookingsData.filter((booking: Booking) => 
        resourcesData.vehicles.some((vehicle: Vehicle) => vehicle.name === booking.resource_name)
      );
  ...
  return (
    <div>
      <h1>차량 예약</h1>
      <p>예약을 원하는 차량을 선택해주세요.</p>
      
      <h2 className="mt-4">차량 목록</h2>
          ...
          <Col key={vehicle.id}>
            <Card>
              <Card.Body>
                ...
                <Button variant="primary" onClick={() => handleShowModal(vehicle)}>
                  예약하기
                </Button>
              </Card.Body>
            </Card>
